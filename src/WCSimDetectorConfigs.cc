...
void WCSimDetectorConstruction::SetNuPrismShort_mPMTGeometry()
{
	...
	// Radius of cone at z=reflectorHeight
	id_reflector_height = 11.67*CLHEP::mm;        // for a radius of 7.25mm, for hex: 5.4mm (radius of 6mm)
	id_reflector_z_offset = 6.12*CLHEP::mm;       //from KM3Net CAD drawings
	id_reflector_angle = 40*CLHEP::deg;         // Need to be remeasured for different PMT curvature 
	mPMT_pmt_openingAngle = 8.2*CLHEP::deg;     // for hex: 8.5deg
	G4double vessel_tot_height = vessel_radius + vessel_cyl_height;
