...
void WCSimDetectorConstruction::SetNuPrismShort_mPMTGeometry()
{
	...
	//mPMT params go first because detector depends on it:
	vessel_cyl_height = 38.*CLHEP::mm;    //option A, option B would be 277 mm
	vessel_radius_curv = 342.*CLHEP::mm;  //needs to include the vessel thickness, as we construct from outside inwards.//me:dome thickness is included.
	vessel_radius = 254.*CLHEP::mm;
	dist_pmt_vessel = 2*CLHEP::mm;      
	orientation = PERPENDICULAR;
	mPMT_outer_material = "G4_PLEXIGLASS";
...
void WCSimDetectorConstruction::SetNuPrismShort_mPMTGeometry()
{
	...
	// Radius of cone at z=reflectorHeight
	id_reflector_height = 8.509*CLHEP::mm;        // for a radius of 7.25mm, for hex: 5.4mm (radius of 6mm)
	id_reflector_z_offset = 1.42*CLHEP::mm;       //from KM3Net CAD drawings
	id_reflector_angle = 49*CLHEP::deg;         // Need to be remeasured for different PMT curvature 
