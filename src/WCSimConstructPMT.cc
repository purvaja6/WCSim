...
G4LogicalVolume* WCSimDetectorConstruction::ConstructPMT(G4String PMTName, G4String CollectionName)
{
	...
	G4double reflectorThickness = 0.33*CLHEP::mm; //me: the actual reflector thickness is 0.5 mm but due to solid works design, I am taking the horizontal component to calculate the reflector radius. Refer my CAD drawing.
	...
	if(id_reflector_height > 0.1*CLHEP::mm
		...
		std::cout << "reflectorradiusmax " << reflectorRadius - 4.715*CLHEP::mm + reflectorThickness << std::endl;

		std::cout << "radiusmin" << radius - 4.715*CLHEP::mm << std::endl;
		std::cout << "radiusmax" << radius - 4.715*CLHEP::mm + reflectorThickness << std::endl;
		G4LogicalVolume* logicReflector =
			new G4LogicalVolume(    reflectorCone,
					G4Material::GetMaterial("Aluminum"), //It actually is Al+ Ag evaporation
					"reflectorCone",
	...
	if(nID_PMTs > 1){
		...
		if(id_reflector_height > 0.1*CLHEP::mm
			...
			G4double ReflectorHolderZ[4] = {0, 22.89, 41.83, 46.69};
			G4double ReflectorHolderR[4] = {39.735, 43.15, 45.90, 46.62};

			G4double ReflectorHolderr[4] = {26.75, 40.52, 40.52, 45.94};
